...
SCRAPING_SEMAPHORE = Semaphore(3)
IMAGE_SEMAPHORE = Semaphore(3)
...
def run_pipeline(asin=None, limit=None, from_csv=None, from_txt=None, skip_existing=True):
    ...
    try:
        ...
        def process_single_asin(current_asin):
            conn = get_connection()
            try:
                logging.info("START ASIN %s", current_asin)
                ...
                logging.info("END ASIN %s", current_asin)
            except Exception as e:
                logging.error(f"Error procesando ASIN {current_asin}: {e}")
            finally:
                conn.close()

        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = [executor.submit(process_single_asin, asin) for asin in asins]

            for future in as_completed(futures):
                try:
                    future.result()
                except Exception as e:
                    logging.error(f"Error en ejecución paralela: {e}")
    finally:
        try:
            conn.close()
        except:
    ...
    logging.info("FIN procesamiento paralelo completado")
